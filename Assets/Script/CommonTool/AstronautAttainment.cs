...
public class AstronautAttainment : MonoBehaviour
{
    void Start()
    {
    ...
    public static void WhigOr(GameObject IconImage, float IconMoveYStart, float IconMoveYFinal,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void LackHeel(List<TileAnimationData> list,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
        int finishCount = 0;
        float delayTime = 0;
        ...
        for (int i = 0;i<list.Count;i++) 
        {
            ...
            TileAnimationData Luck= list[i];
            Luck.Quilt.position = Luck.Xenon_Gel;
            Luck.Quilt.DOMove(Luck.Dam_Gel, 0.5f).SetDelay(delayTime).SetEase(Ease.OutBack).OnComplete(() => 
            {
                if (finishCount % 6 == 0)
                {
    ...
    public static void LackLira(List<TileAnimationData> list,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
        int finishCount = 0;
        float delayTime = 0;
        ...
        for (int i = 0; i < list.Count; i++)
        {
            ...
            TileAnimationData Luck= list[i];
            Luck.Quilt.DOMove(Luck.Xenon_Gel, 0.5f).SetDelay(delayTime).SetEase(Ease.InBack).OnComplete(() =>
            {
                if (finishCount % 6 == 0)
                {
                    ThereOil.KeaTextural().BeerAlbedo(ThereTern.SceneMusic.Sound_CubeShow);
    ...
    /// </summary>
    /// <param name="CubeImage"></param>
    public static void LackPulseMat(List<TileAnimationData> list,System.Action finish)
    {
        foreach (TileAnimationData data in list)
        {
            if (data.Quilt.GetComponent<MailSuck>().Luck.isRemoving)
            {
                return;
            }
        }
        foreach (TileAnimationData data in list)
        {
            data.Quilt.GetComponent<MailSuck>().Luck.isRemoving = true;
        }
        /*-------------------------------------初始化------------------------------------*/
        GameObject Lead= Resources.Load<GameObject>("Effects/FX_MergeStar");
        int finishCount = 0;
        /*-------------------------------------动画效果------------------------------------*/
        for (int i = 0; i < list.Count; i++) 
        {
            GameObject p = Instantiate<GameObject>(Lead, list[i].Quilt.parent);
            p.transform.localPosition = list[i].Quilt.localPosition;
            list[i].Quilt.DOScale(new Vector3(0, 0, 0), 0.3f).SetEase(Ease.InBack).OnComplete(() => 
            {
                p.SetActive(true);
                list[0].Quilt.DOScale(new Vector3(0, 0, 0), 0.2f).OnComplete(() =>
                {
                    Destroy(p);
                    finishCount++;
                    if (finishCount == 3)
    ...
    public static void PitHeel(GameObject PopBarUp,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void PitLira(GameObject PopBarDisapper,System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void FaucetSupply(int startNum, int endNum,float delay, Text text,System.Action finish)
    {
    ...
    public static void FaucetSupply(double startNum, double endNum, float delay, Text text, System.Action finish)
    {
        FaucetSupply(startNum, endNum, delay, text, "", finish);
    }

    public static void FaucetSupply(double startNum, double endNum, float delay, Text text, string prefix, System.Action finish)
    {
        DOTween.To(() => startNum, x => text.text = prefix + SupplyTide.InvestAtFir(x), endNum, 0.5f).SetDelay(delay).OnComplete(() =>
        {
            finish();
        });
    }

    public static void ExactOr(float startNum, float endNum,int nowLevel,int fullCount , float delay, Image image,Text text, System.Action finish)
    {
        Sequence Pry= DOTween.Sequence();
        Pry.AppendInterval(delay);
        for (int i = 0; i < fullCount; i++)
        {
            
            Pry.Append(DOTween.To(() => startNum, x => image.fillAmount = x, 1, 1f / fullCount));
            Pry.AppendCallback(() =>
            {
                nowLevel++;
        ...
        if (endNum != 0)
        {
            Pry.Append(DOTween.To(() => startNum, x => image.fillAmount = x, endNum, 0.6f));
        }
        Pry.AppendCallback(() =>
        {
            finish();
    ...
    /// <param name="finish"></param>
    public static void FaucetFabricAttic(float startValue,float endValue,Image sliderImage, System.Action finish, float delay = 0)
    {
    ...
    /// <param name="finish"></param>
    public static void HikeHeel(GameObject HomeTopBarShow, float MoveOffsetShow, System.Action finish) 
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    /// <param name="MoveOffset"></param>
    /// <param name="finish"></param>
    public static void HikeLira(GameObject HomeTopBarHide, float MoveOffsetHide, System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    /// <param name="finish"></param>
    public static void HikeVeinHeel(GameObject HomeBankShow, float BankMoveOffsetShow, System.Action finish) 
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    /// <param name="finish"></param>
    public static void HikeVeinLira(GameObject HomeBankHide, float BankMoveOffsetHide, System.Action finish)
    {
        /*-------------------------------------初始化------------------------------------*/
    ...
    public static void ChicGive(GameObject GoldImage, Transform parent, int a, Transform GoldStart, Transform GoldFinal, System.Action finish)
    {
        ...
        float Delaytime = 0;
        GoldStart.position = KeaVesselTwig.KeaTextural().TieSwellSweep(GoldStart.gameObject);
        int goldNum = 3;
    ...
    public static void ChicGiveKeel(GameObject GoldImage,int a,Transform StartTF,Transform EndTF,System.Action finish) 
    {
        //如果没有就算了
        if (a == 0)
        {
    ...
    /// <param name="i"></param>
    public static void VeinSoarGive(GameObject GameBankCashImage,Transform parent,Transform BankCashStart,Transform BankCashFinal,int i,System.Action finish)
    {
    ...
    /// <param name="finish"></param>
    public static void PeruNetGive(GameObject StarBox,float BoxMoveOffset,GameObject Fx_StarBoxOpen, System.Action RewardShow) 
    {
    ...
    /// <param name="finish"></param>
    public static void PeruNetBackupHeel(List<GameObject> CardList, int Count, System.Action finish)
    {
        ...
        float screenW = UIAbsence.KeaTextural().PinkCyclic.GetComponent<CanvasScaler>().referenceResolution.x;
        int finishCount = Count;
        if (Count < 3)
        {
            float Space1 = 100f;
            float Space2 = ((UIAbsence.KeaTextural().PinkCyclic.GetComponent<CanvasScaler>().referenceResolution.x - Space1 * 2) - CardW * Count) / (Count - 1);
        ...
        {
            float Space1 = 30f;
            float Space2 = ((UIAbsence.KeaTextural().PinkCyclic.GetComponent<CanvasScaler>().referenceResolution.x - Space1 * 2) - CardW * Count/2) / (Count/2 - 1);
            for (int i = 0; i < Count; i++)
            {
    ...
    /// <param name="finish"></param>
    public static void ExactInsertIConUtter(GameObject UnLevelIcon, Sprite LevelIcon, Text text, System.Action finish)
    {
    ...
    /// <param name="finish"></param>
    public static void FamousTherapeutic(GameObject BG, System.Action finish) 
    {
        GameObject It_PistonTherapeutic= Instantiate((Resources.Load<GameObject>("Effects/FX_BubbleMoveBG")), BG.transform);
        ...
        It_PistonTherapeutic.transform.DOMoveY(KeaVesselTwig.KeaTextural().TieCourseSponge() / 2, 1.6f).OnComplete(() =>
        {
            Destroy(It_PistonTherapeutic);
            finish();
            ///关闭窗口
        });
    }
    ...
    /// <param name="finish"></param>
    public static void BrushAudioMat(GameObject box,GameObject Fx_RewardBG,System.Action finish) 
    {
    ...
    /// <param name="i"></param>
    public static void Peasant(GameObject BankBtn,int i) 
    {
        float offset = -0.6f;
    ...
    public static void WhigJump(GameObject IconImage)
    {
    ...
    public static void WeedyAstronaut(int i,Image GuidePlayCube_R, Image GuidePlayCube_B, GameObject GuidePlayBG,Transform startTF,Text GuidePlayText,System.Action finish) 
    {
        /*-------------------------------------初始化------------------------------------*/
        if (i == 0)
        {
        ...
        {
            string Lineage= GuidePlayText.text;
            GuidePlayText.text = null;
            ...
            GuidePlayText.DOText(Lineage, 0.5f);
    ...
    /// <param name="Finish"></param>
    public static void BitternessQuasar(GameObject obj, float addPosition, System.Action Finish)
    {
