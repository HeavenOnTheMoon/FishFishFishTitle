...
public class SuperBuildWindow : UnityEditor.EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        GUILayout.Space(10);
        LogoTexture = (Texture2D)EditorGUILayout.ObjectField("应用图标", PlayerSettings.GetIconsForTargetGroup(BuildTargetGroup.Unknown)[0], typeof(Texture2D), GUILayout.MaxWidth(800));
        ...
        if (info.DesState == "DES")
        {
            ...
            {
                info.DesState = "STR";
                info.Applovin_SDK_KEY = KeaVesselTwig.CuticleDES(info.Applovin_SDK_KEY);
        ...
        else
        {
            ...
            {
                info.DesState = "DES";
                info.Applovin_SDK_KEY = KeaVesselTwig.PaymentDES(info.Applovin_SDK_KEY);
