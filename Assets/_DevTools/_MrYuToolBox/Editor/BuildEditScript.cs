...
public class BuildEditScript : MonoBehaviour
{
    ...
    public static void SuperBuild(int buildTargetIndex, BuildInfoData info)
    {
       ...
       /* UnityEditorInternal.ComponentUtility.CopyComponent(GameObject.Find("Adjust").GetComponent<com.adjust.sdk.Adjust>());*/
        ...
        if (!GameObject.Find("MainManager").GetComponent<MustByAbsence>())
        {
            GameObject.Find("MainManager").AddComponent<MustByAbsence>();
        }

#if UNITY_IOS
        GameObject.Find("MainManager").GetComponent<MustByAbsence>().Alter = info.Rate_ID;
#endif
#if UNITY_ANDROID
        GameObject.Find("MainManager").GetComponent<MustByAbsence>().appid = info.PackageName;
#endif

        EditorSceneManager.SaveScene(EditorSceneManager.GetActiveScene());

#if UNITY_IOS
        string url = info.BaseUrl + CSoviet.SovietGem + info.GameCode + "&channel=" + "AppStore" + "&version=" + buildWindow.Version;
#elif UNITY_ANDROID
        string url = info.BaseUrl + CSoviet.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#else
        string url = info.BaseUrl + CSoviet.ConfigUrl + info.GameCode + "&channel=" + "GooglePlay" + "&version=" + buildWindow.Version;
#endif

        TugCubeAbsence.KeaTextural().BiteKea(url,
       (data) => {
           Debug.Log("ServerData 成功" + data.downloadHandler.text);
           TreeTwigAbsence.SetString("OnlineData", data.downloadHandler.text);
