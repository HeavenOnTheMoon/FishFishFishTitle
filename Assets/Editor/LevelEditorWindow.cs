    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("查找"))
        {
            ...
            if (levelEditFileInfo.Exists)
            {
                ...
                ExactWorthAsian.KeaTextural().RepeatWorth(limitList);
                ExactWorthAsian.KeaTextural().loadExactSize(fileName);
            }
            ...
            if (levelFileInfo.Exists)
            {
                ...
                figureNum = int.Parse(jsonData.UsefulNum);
            }
        }
        ...
        if (GUILayout.Button("增加8*8"))
        {
            ...
            item.y = 8;
            limitList.Add(item);
            ExactWorthAsian.KeaTextural().RidWorth(item);
        }
        ...
        if (GUILayout.Button("删除最后一层"))
        {
            ...
            ExactWorthAsian.KeaTextural().NinetyWorth();
        }
        GUILayout.Space(50);
        ...
        {
            if (ExactWorthAsian.KeaTextural().transform.childCount == 0)
            {
                ExactWorthAsian.KeaTextural().RepeatWorth(limitList);
            }
            else
            {
                if (EditorUtility.DisplayDialog("提示", "重新生成编辑地图可能会丢失当前编辑记录", "确认", "取消"))
                {
                    ...
                    {
                        limitList[i].SenatorFatal = false;
                    }
                    ExactWorthAsian.KeaTextural().RepeatWorth(limitList);
                }
            }
        ...
        if (GUILayout.Button("保存Json"))
        {
            ...
            if (ExactWorthAsian.KeaTextural().ThawStyle % 3 != 0)
            {
            ...
            else
            {
                ...
                if (!levelFileInfo.Exists)
                {
                    ExactWorthAsian.KeaTextural().VineExactSize(fileName, figureNum.ToString());
                }
                else
                {
        ...
        GUILayout.Space(20);
        if (ExactWorthAsian.KeaTextural())
        {
            fullCount = "当前激活方块数:" + ExactWorthAsian.KeaTextural().ThawStyle + "\n余数:" + ExactWorthAsian.KeaTextural().ThawStyle % 3;
            GUILayout.Label(fullCount);
        }
        ExactWorthAsian.KeaTextural().PrepayFatalAlpha(limitList);
...
public void ChangeLevelImageCount()
{
    ...
    var levelEditFiles = levelEditInfo.GetFiles();
        ...
        for (int i = 0; i < levelEditFiles.Length / 2; i++)
        {
            ...
            int figureNum = int.Parse(jsonData.UsefulNum);
            ...
            string levelJsonData = JsonUtility.ToJson(new LevelJsonData(jsonData.Wealth, figureNum.ToString()));
    ...
    public void checkLevelWithLayerAndCount()
    {
        ...
        for (int i = 0; i < levelEditFiles.Length/2; i ++)
        {
            ...
            if (limitList.Count >= int.Parse(checkLayer) && jsonData.Wealth.Count >= int.Parse(checkCount))
            {
                levelString += "\n" + (1 + i);
                levelCount++;
            }
    ...
    public void removeLevel()
    {
        ...
        if (levelRemoveFileInfo.Exists)
        {
            ...
            foreach (LevelRemoveItem item in removeData.Ninety_Mine)
            {
                foreach(LevelRemoveItem item2 in allItemList)
                {
                    if (item2.ExactShy == item.ExactShy)
                    {
            ...
            for (int i = 0; i < resetItemList.Count; i++)
            {
                ...
                string oldLevelFilepath = levelFilepath + "/Level_" + item.ExactShy +".json";
                string newLevelFilepath = levelFilepath + "/Level_" + (i+1) +".json";
                string oldLevelEditFilepath = levelEditFilepath + "/Level_Edit_" + item.ExactShy + ".json";
